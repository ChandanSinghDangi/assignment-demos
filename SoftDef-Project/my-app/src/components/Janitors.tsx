...
const JanitorsSchedule: React.FC<JanitorsScheduleProps> = ({dark}) => {
  ...
  return (
    ...
    <div className={`p-1 border rounded-lg h-full ${dark ? 'bg-[#472a52] border-[#795088]' : 'bg-gray-100 border-gray-200'}`}>
              ...
              <div key={employee.id} className=''>
                {employee.schedule.map((week, weekIndex) => (
                  <div key={`${employee.id}-${weekIndex}`} className={`grid grid-cols-[100px_1fr] 
                    ...
                    <div className={`p-4 rounded border-r ${dark ? 'border-[#795088] bg-[#54345f]' : 'border-gray-200 bg-white'}`}>
                        {weekIndex === 0 ? (
...
export default JanitorsSchedule;
